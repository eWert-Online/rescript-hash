let make = (message: Bytes.t) => {
  ...
  let a = ref(ReCrypt_Sha256_Constants.initialHash[0]);
  let b = ref(ReCrypt_Sha256_Constants.initialHash[1]);
  let c = ref(ReCrypt_Sha256_Constants.initialHash[2]);
  let d = ref(ReCrypt_Sha256_Constants.initialHash[3]);
  let e = ref(ReCrypt_Sha256_Constants.initialHash[4]);
  let f = ref(ReCrypt_Sha256_Constants.initialHash[5]);
  let g = ref(ReCrypt_Sha256_Constants.initialHash[6]);
  let h = ref(ReCrypt_Sha256_Constants.initialHash[7]);
  ...
  for (i in 0 to chunks - 1) {
    ...
    for (t in 0 to 63) {
      let temp1 =
        ...
        ->Int32.add(ReCrypt_Sha256_Constants.k[t])
        ->Int32.add(w[t]);
